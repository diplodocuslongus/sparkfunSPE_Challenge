...
float reported_maxamplitude; // max amplitude of vibration/wave/shock
...
void loop() {
    ...
    float maxamplitude = (float)random(30); //  mySensor.readFloatHumidity();
    float wvfreq = (float)random(20); // mySensor.readFloatWaveFreq();
    float alt = (float)random(10); // mySensor.readFloatAltitudeFeet();
    float temp = (float)random(40); // mySensor.readTempF();
    int maxamplitude_dec = ( (int)(maxamplitude*100) ) % 100;
    int wvfreq_dec = ( (int)(wvfreq*100) ) % 100;
    int alt_dec = ( (int)(alt*100) ) % 100;
    int temp_dec = ( (int)(temp*100) ) % 100;

    if(diff(maxamplitude, reported_maxamplitude, 5)) force_report = true;
    /*
    if(diff(wvfreq, reported_wvfreq, 300)) force_report = true;
