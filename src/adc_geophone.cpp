...
void setup(void)
{
  ...
  mySensor.setGain(ADS122C04_GAIN_2); // Set the gain to 1
  ...
  mySensor.setDataRate(ADS122C04_DATA_RATE_175SPS); // Set the data rate (samples per second) to 20 or 45 or 90 or 175 or...
...
void loop()
{
  ...
  {
  uint32_t raw_ADC_data = mySensor.readADC(); // original
  //int32_t raw_ADC_data = (int32_t) mySensor.readADC();
